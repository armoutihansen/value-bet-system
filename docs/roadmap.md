...
- **🚦 GO / NO-GO GATE:**
  - *Necessary:* the model is calibrated on a strict walk-forward.
  - *Go:* ≥ 1 pre-registered slice (league / scoring environment) where the model's log-loss **matches or beats** the de-vigged close.
  - *No-go:* calibrated but uniformly worse than the close → publish the honest "efficient market" result; **stop or pivot.** Do not spend on data or build the betting layer.

## Phase 1.5 — measured refinements *(only if M1 is promising)*

Add features (ClubElo ratings, lagged Understat xG), test the Dixon-Coles τ-correction, and run the challenger (Bayesian hierarchical dynamic Poisson; gradient-boosting). **Every change is A/B'd against the M1 Poisson floor** — adopt only what demonstrably improves calibration / model-vs-close.
